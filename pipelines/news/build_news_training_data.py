...
from config.db_config import MONGO_URI, DB_NAME, COLLECTION_HOURLY_NEWS_SENTIMENT, COLLECTION_STOCK_PRICES


def load_news_sentiment_by_ticker(db, ticker):
    """Load hourly news sentiment features for specified ticker."""
    ...
    feature_cols = [
        'p_pos', 'p_neg', 'p_neu',
        'mean_sentiment', 'news_count', 'sentiment_std',
...
def extract_72hour_window(sentiment_df, prediction_time):
    """Extract 72-hour news sentiment feature window."""
...
def build_training_samples(db, prediction_hour=16):
    ...
    if not tickers:
        print("   ERROR: No tickers found in hourly_news_sentiment collection.")
        print("   Please run aggregate_news.py first.")
    ...
    print(f"   Features: [p_pos, p_neg, p_neu, mean_sent, count, std, max, min]")
    ...
    for ticker in tickers:
        ...
        sentiment_df = load_news_sentiment_by_ticker(db, ticker)
        stock_df = load_financial_data_by_ticker(db, ticker)
        
        if sentiment_df is None:
            print(f"     No news sentiment data for {ticker}")
            continue
        ...
        print(f"     News sentiment: {len(sentiment_df)} hours")
...
def save_training_data(X, y, dates, tickers, output_file):
    ...
    print("     data = np.load('news_lstm_training_data.npz')")
...
if __name__ == "__main__":
    output_file = 'news_lstm_training_data.npz'
    
    print("=" * 70)
    print("News LSTM Training Data Builder")
    print("=" * 70)
    ...
    if X is None or len(X) == 0:
        ...
        print("\nPossible solutions:")
        print("  1. Run aggregate_news.py to create hourly aggregations")
        print("  2. Ensure you have stock price data in the stock_prices collection")
        print("  3. Verify temporal overlap between news sentiment and stock prices")
        print("\nNote: Sparse news data distributed across days/weeks")
